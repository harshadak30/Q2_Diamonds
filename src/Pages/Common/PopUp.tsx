...
const PopUp: React.FC<PopUpProps> = ({ isOpen, onClose, images }) => {
  ...
  return (
    ...
    >
      ...
      `}</style>
      <div 
        className="bg-white rounded-lg w-full max-w-md md:max-w-6xl relative shadow-2xl overflow-hidden my-auto"
          ...
          {images.map((image, idx) => (
            ...
            >
              <img 
                src={image} 
                alt={`Image ${idx + 1}`}
                className="w-full h-auto md:h-[75vh] object-contain"
              />
            </div>
          ))}
        </div>
